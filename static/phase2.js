/**
 * Railway Route Circuit Visualization System
 * Phase 2 JavaScript functionality
 * 
 * This script manages interactive features for the route visualization interface
 */

// Application configuration
const config = {
    apiPrefix: '/phase2',  // API endpoint prefix
    toast: {
...
function showToast(message, type) {
    ...
    setTimeout(() => {
        ...
        setTimeout(() => $('.toast-notification').remove(), config.toast.animationDuration);
...
function handleVisualizationSuccess(response) {
    ...
    if (response.plot) {
        ...
        // Adding a slight delay to ensure the plot is fully rendered
        setTimeout(function() {
            initKeyboardNavigation();
            
            // If we're in fullscreen mode, resize the plot to fit
            if ($('.viz-card').hasClass('fullscreen')) {
                resizePlotlyPlot();
                
                // Trigger second resize after a delay to ensure accurate sizing
                setTimeout(resizePlotlyPlot, 500);
            }
        }, 500);
        ...
        if (response.stats) {
            ...
            $('#avgSpeed').text((response.stats.avgSpeed || '0') + ' km/h');
        }
        
        // Update movement counts by route - pass the selected routes
...
function updateMovementCountsByRoute(movementCounts, selectedRoutes) {
    ...
    // Create a complete list of routes - all that are selected
    // For any selected route not in movementCounts, add it with count 0
    ...
    // Sort routes alphabetically for consistent display
    const sortedRoutes = allRoutes.sort();
    
    // Create a card for each route
    sortedRoutes.forEach((routeId, index) => {
        const count = movementCounts[routeId] || 0; // Use 0 if the route has no data
        const color = colors[index % colors.length];
        
        // Make the card clickable and add data-route attribute for filtering
        ...
        card.on('click', function() {
            // Filter movement times to show only this route
...
function setupFormSubmission() {
    $('#visualizationForm').on('submit', function(e) {
        ...
        // Get form values
        const routes = $('#route').val(); // This now returns an array with multiple select
        ...
        // Generate visualization with multiple routes
        generateVisualization(routes, fromTime, toTime);
        
        // Also fetch and display movement times
        fetchMovementTimes(routes, fromTime, toTime);
    });
    
    // Update the selected routes display when selection changes
...
function setupUIControls() {
    ...
    // Setup keyboard navigation for rangeslider
    setupRangeSliderKeyboardNavigation();
}

// Remove resizePlotlyPlot() function
...
function setupRangeSliderKeyboardNavigation() {
    ...
    $(document).off('keydown.rangeslider').on('keydown.rangeslider', function(e) {
        ...
        // Don't intercept arrow keys if user is typing in a form field
        ...
        if (key === 37 || key === 39) {
            ...
            if (!gd || !gd._fullLayout || !gd._fullLayout.xaxis) {
                console.warn('Cannot shift view: Plot not found or not ready');
                return;
            }
            
            console.log('Processing keyboard navigation', key === 37 ? 'left' : 'right');
            e.preventDefault(); // Prevent default scrolling
            
            const direction = (key === 37) ? -1 : 1; // -1 for left, 1 for right
...
function shiftRangesliderView(direction, percentage) {
    ...
    if (!gd) {
        console.warn('Cannot shift rangeslider: Plot element not found');
        return;
    }
    
    if (!gd._fullLayout || !gd._fullLayout.xaxis) {
        console.warn('Cannot shift rangeslider: Plot layout not ready');
        return;
    }
    ...
    if (!Array.isArray(xaxis.range) || xaxis.range.length !== 2) {
        console.warn('Cannot shift rangeslider: Invalid axis range', xaxis.range);
        return;
    }
    
    console.log('Current range:', xaxis.range);
    
    // Convert range values to dates (they might be timestamps or date strings)
    const currentRange = xaxis.range.map(r => new Date(r));
    
    // Calculate the time span of the current view
    const timeSpan = currentRange[1] - currentRange[0]; // in milliseconds
    
    if (!timeSpan || isNaN(timeSpan)) {
        console.warn('Cannot shift rangeslider: Invalid time span');
        return;
    }
    ...
    console.log('Shifting rangeslider', direction > 0 ? 'right' : 'left', 'by', percentage * 100, '%');
    console.log('New range:', newRange.map(d => d.toISOString()));
    
    // Update the range with animation
    Plotly.relayout(gd, {
        'xaxis.range': newRange
    }).then(() => {
        // Show a subtle indicator that the view has shifted
...
function initKeyboardNavigation() {
    ...
    // Make sure visualization container can receive focus for keyboard events
    const vizContainer = document.getElementById('visualization');
    if (vizContainer && !vizContainer.hasAttribute('tabindex')) {
        vizContainer.setAttribute('tabindex', '0');
    }
    
    // Ensure the range slider keyboard navigation is properly set up
    setupRangeSliderKeyboardNavigation();
    
    // Add navigation buttons for the timeline
    addTimelineNavigationButtons();
    
    // Set focus to the visualization container to enable keyboard navigation
    // but only if we're not currently focused on an input field
    if (!$('input:focus, select:focus, textarea:focus').length) {
        setTimeout(() => {
            if (vizContainer) {
                vizContainer.focus({preventScroll: true});
                console.log('Visualization container focused for keyboard navigation');
                
                // Add a visual indicator for keyboard navigation
                showKeyboardNavigationHint();
            }
        }, 500);
    }
    
    // Also listen for Plotly's afterplot event for better timing
    $(document).on('plotly_afterplot', function() {
        console.log('Plotly afterplot event detected, ensuring keyboard navigation is ready');
        setupRangeSliderKeyboardNavigation();
    });
}
...
function addTimelineNavigationButtons() {
    console.log('Adding timeline navigation buttons');
    ...
    // Add buttons to the visualization container
        ...
        if (window.Plotly && document.querySelector('.js-plotly-plot')) {
            Plotly.downloadImage(
                document.querySelector('.js-plotly-plot'), 
                {
                    ...
                    width: 1200, 
                    height: 800, 
                    filename: 'railway_visualization'
                }
            );
        } else {
...
function setupRouteTooltip() {
    // Show route info tooltip when clicking the routes stats card
    $('#routesStatsCard').on('click', function() {
        console.log('Routes stats card clicked, showing tooltip');
        // Fetch and show all routes
...
function showAllRoutesTooltip() {
    ...
    $.getJSON(`${config.apiPrefix}/routes`)
        .done(function(data) {
            if (data.routes && data.routes.length > 0) {
                // Create a backdrop for the tooltip
                $('body').append('<div class="tooltip-backdrop"></div>');
                
                // Get route details from the Route_chart.csv data
                        ...
                        fetch(`${config.apiPrefix}/route_details`)
                            ...
                            .then(routeDetails => {
                                ...
                                data.routes.forEach(routeId => {
                                    ...
                                    // Show route name if available with improved visibility
                            ...
                            .catch(error => {
                                ...
                                // Continue with just the circuit data if route details can't be loaded
...
function showTooltipWithoutRouteNames(routes, routeData) {
    ...
    // Generate HTML for all routes (without route names)
    let routesHtml = '';
...
function formatCircuitPath(circuitPath) {
    ...
    // Split the circuit path by the delimiter and wrap each segment
...
function fetchMovementTimes(routes, fromTime, toTime) {
    // Show loading state in the movement times container
    ...
    // Make API request to get movement times for all selected routes
    $.ajax({
 ...
 * @param {Object} data - Movement times data from APIs
 */
function displayMovementTimes(data) {
    ...
    console.log(`Fetched ${movements.length} movement times`);
    
    // Count total movements per route and collect unique routes
    const routeCounts = {};
    ...
    // Create table header with just the title (no filter buttons)
    ...
    // Add click handler for rows to show more details
...
function setupFileUpload() {
    ...
    $('#routeChartFile, #trackCircuitFile').on('change', function() {
        ...
        const statusId = fileId === 'routeChartFile' ? 'routeChartStatus' : 'trackCircuitStatus';
...
function handleUploadSuccess(response) {
    if (response.route_chart_status) {
        $('#routeChartStatus').html(`<span class="text-success"><i class="fas fa-check-circle"></i> ${response.route_chart_status}</span>`);
    } else {
        $('#routeChartStatus').html('<span class="text-danger"><i class="fas fa-exclamation-circle"></i> Route chart upload failed</span>');
    }
    if (response.track_circuit_status) {
        $('#trackCircuitStatus').html(`<span class="text-success"><i class="fas fa-check-circle"></i> ${response.track_circuit_status}</span>`);
    } else {
        $('#trackCircuitStatus').html('<span class="text-danger"><i class="fas fa-exclamation-circle"></i> Track circuit upload failed</span>');
    }
    
    // Check if system is ready with all required files
    if (response.system_ready) {
        showToast('Files uploaded successfully! Refreshing available routes...', 'success');
    } else {
        // Show warning if still missing files
        if (response.missing) {
            showToast(`Files uploaded but: ${response.missing}`, 'warning');
        } else {
            showToast('Files uploaded. Please upload any missing required files.', 'warning');
        }
    }
    
    // Refresh the routes list
    setTimeout(populateRouteDropdown, 1000);
}
...
function resetFileUploads() {
    ...
    // Also tell the server to reset to default files
    $.ajax({
        url: `${config.apiPrefix}/reset_files`,
        type: 'POST',
        success: function() {
            showToast('Files reset successfully. Refreshing route list...', 'success');
            setTimeout(populateRouteDropdown, 1000);
        },
        error: function() {
...
$(document).ready(function() {
    console.log("Phase 2 visualization module initializing...");
    ...
    setupDownloadButton();
    setupRouteTooltip();
    setupFileUpload(); // Add the new file upload setup
    
    // Enhanced Plotly event listeners for keyboard navigation
    $(document).on('plotly_afterplot', function() {
        console.log('Plotly chart rendered, setting up keyboard navigation');
        initKeyboardNavigation();
    });
    
    // Add CSS for keyboard navigation hints, indicators, and navigation buttons
    ...
    console.log("Phase 2 module initialization complete");
});

// Function to populate the route dropdown
function populateRouteDropdown() {
    $('#route').empty().append('<option value="">Select a route...</option>');
    
    // Show loading indicator
    $('#route').attr('disabled', true);
    $('#route').append('<option value="" disabled>Loading routes...</option>');
    ...
    fetch('/phase2/routes')
        ...
        .then(data => {
            ...
            // Remove loading message and enable select
            $('#route').empty().append('<option value="">Select a route...</option>');
            $('#route').attr('disabled', false);
            
            // Check if routes array exists and has items
            if (data.routes && Array.isArray(data.routes) && data.routes.length > 0) {
                ...
                $('#route').append('<option value="" disabled>No routes available</option>');
                console.error('No routes returned from API:', data);
            }
        })
        .catch(error => {
            console.error('Error fetching routes:', error);
            $('#route').attr('disabled', true);
            $('#route').empty().append('<option value="">Error loading routes</option>');
        });
}
